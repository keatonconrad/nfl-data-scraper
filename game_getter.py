...
class GameGetter:
    ...
    def get_player_stats(self, res: HTMLSession, team_stats_obj: dict) -> list[dict]:
        ...
        for stat in player_stats:
            if stat in stat_section_mapping:
            ...
            elif team_stats_obj['away_team'] in stat or team_stats_obj['home_team'] in stat:
                ...
                if player not in player_stats_obj:
                    player_stats_obj[player] = {'date': team_stats_obj['date'], 'team': team_stats_obj[f'{team}_team']}
                i = 0
    ...
    def get_team_stats(self, res: HTMLSession) -> dict:
        ...
        attendance_index = next((i for i, info in enumerate(game_info) if 'Attendance' in info), -1)
        ...
        for i in range(1, 6):
            team_stats_obj[f'away_score_q{i}'] = score_line[score_offset + i - 1] if i <= 4 or is_overtime else '0'
            team_stats_obj[f'home_score_q{i}'] = score_line[score_offset + i + 6 - 1] if i <= 4 or is_overtime else '0'
    ...
    def get_games(self, start_year: int, last_year_start_week: int) -> tuple[pd.DataFrame]:
        ...
        for year in tqdm(range(start_year, self.current_season + 1), desc='Years'):
            ...
            with ThreadPoolExecutor() as executor:
                future_to_url = {executor.submit(self.process_game, url): url for url in game_links}
    ...
    def query_game_url(self):
        return self.session.get(f'https://www.footballdb.com/games/index.html?lg=NFL&yr={self.current_season}')
